                        ...
                        <div class="col-12">
                            <div class="card">
                                <div class="card-header">
                                    ...
                                    @can('offer-add')
                                    <a href="{{route('popular-events-news.create')}}" class="btn btn-primary btn-print mb-1 mb-md-0 waves-effect waves-light"><i class="feather icon-plus-circle"></i>&nbsp;Add Popular Events News</a>
                                    @endcan
                                </div>
                                    ...
                                    <div class="card-body card-dashboard">
                                        <div class="table-responsive">
                                            <table class="table zero-configuration">
                                                ...
                                                @if(count($popularEventsNews) > 0)
                                                    @foreach($popularEventsNews as $event)
                                                        <tr>
                                                            <td>{{$event->name_en}} </td>
                                                            <td>{{$event->name_ar}} </td>
                                                            {{-- <td class="product-img"><img
                                                                    ...
                                                                    width="50"/></td> --}}
                                                            <td>
                                                                @can('popular-events-news-edit')
                                                                    <a href="{{ route('popular-events-news.edit', $event->id) }}"><i
...
@section('extrascript')
    <script>
        ...
        function destroy(delId) {
            let url = '{{ route("popular-events-news.destroy", ":id") }}';
