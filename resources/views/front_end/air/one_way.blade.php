    ...
    <section class="page-header page-header-dark bg-secondary">
      <div class="container">
        <div class="row align-items-center">
          <div class="col-md-8">
            <h1>{{__('lang.flights_one_way')}} </h1>
          </div>
          <div class="col-md-4">
            <ul class="breadcrumb justify-content-start justify-content-md-end mb-0">
              <li><a href="{{url('/')}}">{{__('lang.home')}}</a></li>
              <li><a href="{{url('/')}}">{{__('lang.flights')}}</a></li>
              <li class="active">{{__('lang.flights_one_way_breadcurm')}}</li>
            </ul>
          </div>
        </div>
      </div>
    ...
    <section class="container">
      <form id="bookingFlight" method="get" action="{{route('SearchFlights')}}">
        ...
        <div class="row g-3 mb-4">
          ...
          <div class="col-md-6 col-lg-2">
            <div class="position-relative">
              <input id="flightReturn" type="text" class="form-control"  placeholder="{{__('lang.return_date')}}" name = "ReturnDate" autocomplete="off" value="{{app('request')->input('ReturnDate')}}" disabled>
              <span class="icon-inside"><i class="far fa-calendar-alt"></i></span> </div>
          </div>
          <div class="col-md-6 col-lg-2">
        ...
        <div class="col-md-9 mt-4 mt-md-0">
          <div class="bg-white shadow-md rounded py-4">
            <div class="mx-3 mb-3 text-center">
              <h2 class="text-6 mb-4">{{$data['OriginCityDetails']->city_name}} <small class="mx-2">to</small>{{$data['DestationCityDetails']->city_name}}</h2>
            </div>
            <div class="text-1 bg-light-3 border border-end-0 border-start-0 py-2 px-2">
              <div class="row">
                <div class="col col-sm-2 text-center"><span class="d-none d-sm-block">{{__('lang.airlines')}} </span></div>
                <div class="col col-sm-2 text-center">{{__('lang.departure')}} </div>
            ...
            <div class="flight-list">
              ...
              <div class="flight-item" data-pricing = "{{$info['markupPrice']['totalPrice']['value']}}" data-airline = "{{clean($info['airline'])}}" data-connection = "{{clean(Connections($info['connections']))}}" >
                <div class="row align-items-center flex-row pt-4 pb-2 px-2">
                  <div class="col col-sm-2 text-center d-lg-flex company-info"> 
                    <span class="align-middle">
                      <img class="" alt="" width="40" height="40" src="https://www.gstatic.com/flights/airline_logos/70px/{{$info['IATACODE']}}.png"> 
                    </span>
                    <span class="align-middle ms-lg-2">
                      <span class="d-block text-1 text-dark">{{$info['airline']}}
                       <small class="text-muted d-block">
                            @if($info['itinerary'][0]['AirSegment']['Carrier'] == "G9")
                            {{$info['itinerary'][0]['AirSegment']['FlightNumber']}}
                            @else
                            {{$info['itinerary'][0]['AirSegment']['Carrier']}}-{{$info['itinerary'][0]['AirSegment']['FlightNumber']}}
                            @endif
                                @if($info['itinerary'][0]['AirSegment']['Carrier'] == "J9")
                            <br>{{$info['itinerary'][0]['baggage']['product']}}
                            @endif
                        </small>
                      </span> 
                    </span>
                  ...
                  <div class="col col-sm-2 text-center time-info"> 
                    <span class="text-4">{{$info['depatureTime']}}</span> 
                    <div>
                      <small class="text-muted d-sm-block listingAirportName">{{$info['depatureDate']}}</small>
                      <small class="text-muted d-sm-block ">{{$info['OriginAirportDetails']->name}}</small> 
                    </div>
                  </div>
                  <div class="col-sm-2 text-center d-none d-sm-block time-info"> <span class="text-3">{{$info['totalTimeTravel']}}</span> <small class="text-muted d-none d-sm-block">{{Connections($info['connections'])}}</small> </div>
                  <div class="col col-sm-2 text-center time-info"> 
                    <span class="text-4">{{$info['arrivalTime']}}</span> 
                    <div>
                      <small class="text-muted d-sm-block listingAirportName">{{$info['arrivalDate']}}</small>
                      <small class="text-muted d-sm-block ">{{$info['DestationAirportDetails']->name}}</small> 
                    </div>
                    
                  </div>
                  <div class="col col-sm-2 text-end text-dark text-3 price">
                  ...
                  <div class="col-12 col-sm-2 text-center ms-auto btn-book"> 
                  ...
                  <div class="col col-sm-auto col-lg-10 mt-1 text-1 ">{{$info['layover']}}</div>
                  <div class="col col-sm-auto col-lg-2 ms-auto mt-1 text-1 text-center"><a data-bs-toggle="modal" data-bs-target="#flight-{{$loop->iteration}}" href="">{{__('lang.flight_details')}}</a></div>
                </div>
                      ...
                      <div class="modal-body">
                        <div class="flight-details">
                          <div class="row mb-4">
                            <div class="col-12 col-sm-9 col-lg-7">
                              <div class="row align-items-center trip-title mb-3">
                                <div class="col col-sm-auto text-center text-sm-start">
                                  <h5 class="m-0 trip-place">{{$info['OriginAirportDetails']->city_name}}</h5>
                                  <p class="m-0 trip-place">{{$info['OriginAirportDetails']->name}}</p>
                                </div>
                                <div class="col-auto text-10 text-black-50 text-center trip-arrow">➝</div>
                                <div class="col col-sm-auto text-center text-sm-start">
                                  <h5 class="m-0 trip-place">{{$info['DestationAirportDetails']->city_name}}</h5>
                                  <p class="m-0 trip-place">{{$info['DestationAirportDetails']->name}}</p>
                                </div>
                              </div>
                              <div class="row align-items-center">
                                <div class="col col-sm-auto"><span class="text-4">
                                  {{date('d M D', strtotime($info['depatureDate']))}} ,{{date('d M D', strtotime($info['arrivalDate']))}} </span></div>
                                <div class="col-auto">
                            ...
                            <div class="col-12 col-sm-3 col-lg-3 text-center text-sm-end mt-3 mt-sm-0" style="margin-top: 32px !important;"> <span class="text-dark text-5">
                            ...
                            {{-- <div class="col-12 col-sm-3 col-lg-2  btn-book d-flex align-items-center" style ="    padding-left: 29px;"> <a href="{{route('flight-review')}}" class="btn btn-sm btn-primary"><i class="fas fa-shopping-cart d-block d-lg-none"></i> <span class="d-none d-lg-block">Book</span></a> </div> --}}
                          ...
                          <div class="tab-content my-3" id="myTabContent">
                            <div class="tab-pane fade show active" id="first{{$loop->iteration}}" role="tabpanel" aria-labelledby="first-tab{{$loop->iteration}}">
                              @foreach($info['itinerary'] as $k=>$segment)
                              <?php //dd($segment)?>
                              <div class="row flex-row pt-4 px-md-4">
                                <div class="col-12 col-sm-3 text-center d-lg-flex company-info"> <span class="align-middle"><img class="" alt="" src="https://www.gstatic.com/flights/airline_logos/70px/{{$segment['AirSegment']['Carrier']}}.png"> </span> <span class="align-middle ms-lg-2"> <span class="d-block text-1 text-dark">{{$segment['AirSegment']['airline']}}</span> <small class="text-muted d-block">
                                @if($segment['AirSegment']['Carrier'] == "G9")
                                {{$segment['AirSegment']['FlightNumber']}}
                                @elseif($segment['AirSegment']['Carrier'] == "J9")
                                {{$segment['AirSegment']['FlightNumber']}}
                                @else
                                {{$segment['AirSegment']['Carrier']}}-{{$segment['AirSegment']['FlightNumber']}}
                                @endif
                                </small> </span> </div>
                                <div class="col-12 col-sm-3 text-center time-info mt-3 mt-sm-0"> 
                                  <span class="text-5 text-dark">{{$segment['AirSegment']['cleanDepartureTime']}}</span>
                                  <small class="text-muted d-none d-sm-block">{{$segment['AirSegment']['cleanDepartureDate']}}</small>
                                  <small class="text-muted d-block">{{$segment['AirSegment']['OriginAirportDetails']->name}}, {{$segment['AirSegment']['OriginAirportDetails']->city_name}} ,{{$segment['AirSegment']['OriginAirportDetails']->country_name}}</small> 
                                </div>
                                <div class="col-12 col-sm-3 text-center time-info mt-3 mt-sm-0"> <span class="text-3 text-dark">{{$segment['AirSegment']['FlightTravelTime']}}</span> <small class="text-muted d-block">{{__('lang.duration')}}</small> </div>
                                <div class="col-12 col-sm-3 text-center time-info mt-3 mt-sm-0"> <span class="text-5 text-dark">{{$segment['AirSegment']['cleanArrivalTime']}}</span> 
                                  <small class="text-muted d-none d-sm-block">{{$segment['AirSegment']['cleanArrivalDate']}}</small>
                                  <small class="text-muted d-block">{{$segment['AirSegment']['DestationAirportDetails']->name}}, {{$segment['AirSegment']['DestationAirportDetails']->city_name}} ,{{$segment['AirSegment']['DestationAirportDetails']->country_name}}</small>  </div>
                              </div>
                                <!-- Original code before  -->
                                {{--@if(isset($info['itinerary'][$k+1]['AirSegment']['LayOverTime'] }}))
                                <div class="dF width100 padTB5 marginTB20 alignItemsCenter backgroundLn justifyBetween"><hr width="400" style="border: 2px dashed #C0C0C0" color="#FFFFFF" size="6"><span class="paleGreyBg brdrRd10 padLR15 padTB3 ico11 blueGrey text-center"style="    width: inherit;"><span class="fb">{{$segment['LayOverTime']." Layover"}}</span></span><hr width="400" style="border: 2px dashed #C0C0C0" color="#FFFFFF" size="6"></div>
                                @endif--}}
                                 <!-- Modified code -->
                                @if(isset($info['itinerary'][$k+1]['AirSegment']['LayOverTime']) || isset($segment['LayOverTime']))
                                  @php
                                    $layoverTime = $info['itinerary'][$k+1]['AirSegment']['LayOverTime'] ?? $segment['LayOverTime'];
                                  @endphp
                                  @if(!$loop->last)
                                  <div class="dF width100 padTB5 marginTB20 alignItemsCenter backgroundLn justifyBetween"><hr width="400" style="border: 2px dashed #C0C0C0" color="#FFFFFF" size="6"><span class="paleGreyBg brdrRd10 padLR15 padTB3 ico11 blueGrey text-center"style="    width: inherit;"><span class="fb">{{$layoverTime." Layover"}}</span></span><hr width="400" style="border: 2px dashed #C0C0C0" color="#FFFFFF" size="6"></div>
                                  @endif
                                @endif
                              @endforeach
                                  ...
                                  <table class="table table-hover table-bordered">
                                    ...
                                    <tbody>
                                      <tr>
                                        ...
                                        <td class="text-center">{{$info['CancelPenalty']}}</td>
                                      </tr>
                                ...
                                <div class="col-6">
                                  <table class="table table-hover table-bordered">
                                    ...
                                    <tbody>
                                      <tr>
                                        <td>ChangePenalty</td>
                                        <td class="text-center">{{$info['ChangePenalty']}}</td>
                                      </tr>
